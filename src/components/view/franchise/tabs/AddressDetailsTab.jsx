...
const AddressDetailsTab = ({ projectId }) => {
    ...
    const getAddressDetails = async () => {
        try {
            const response = await MakeApicallWithoutToken(`address/${projectId}`, 'GET');
            if (response?.success) {
                    ...
                    field.name == 'city' ?
                        newProject ?
                            <Tooltip title={allData.state != '' ? '' : 'City will be auto-selected'} placement='right'>
                                <Input
                                    // defaultValue={newProject ? '' : allData?.city}
                                    style={{ width: 300 }}
                                    type="text"
                                    name={field.name}
                                    value={allData?.state}
            ...
            case "textarea":
                return (
                    <div className='flex flex-col gap-2'>
                        <Input.TextArea defaultValue={newProject ? '' : allData?.street1} rows={2} placeholder='Street 1' style={{ width: 500 }}
                            onChange={(e) => handleChange(e, null, 'address1')}
                        />
                        <Input.TextArea defaultValue={newProject ? '' : allData?.street2} rows={2} placeholder='Street 2' style={{ width: 500 }}
                            onChange={(e) => handleChange(e, null, 'address2')}
                        />
                    </div>
                );
            case "dropdown":
                return (
                    field.name == 'country' ?
                        ...
                        // defaultValue={newProject ? '' : allData?.country}
                        /> :
                        !newProject ?
                            <Select
                                showSearch
                                style={{ width: 300 }}
                                placeholder={'Select'}
                                // defaultValue={newProject ? '' : allData?.state}
                                options={allCities}
                                onChange={(e) => handleChange(null, e, field.name)}
                            />
